...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(curStage)
		{
			case 'waza':
				...
				evilTrail = new FlxTrail(dadOpponent, null, 4, 24, 0.3, 0.069); //nice
				insert(members.indexOf(dadOpponent) - 1, evilTrail);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case 'waza-pelicula':
				dadOpponent.y += (Math.sin(elapsedtime) * 0.6);
				...
				switch(curStep)
				{
					...
					case 1343:
						...
						holaDani2 = false;
					case 1439:
						FlxTween.tween(dadOpponent, {alpha: 0}, 5);
				...
				case 'waza-pelicula':
					if(character == dadOpponent) {
						...
						if(!Init.trueSettings.get('Reduced Movements')) {
							for (hud in allUIs) hud.shake(0.0025, 0.095);
							FlxG.camera.shake(0.0045, 0.065);
						}
						//shake le camera - sexo

						uiHUD.iconP2.setGraphicSize(Std.int(uiHUD.iconP2.width + 15.5));
	...
	override function beatHit()
	{
		...
		if (camZoomSnap && !Init.trueSettings.get('Reduced Movements')) {
		...
		if (camZoomPENE && !Init.trueSettings.get('Reduced Movements')) {
			if (FlxG.camera.zoom < 1.35) {
			   FlxG.camera.zoom += 0.015;
			   for (hud in allUIs)
				    hud.zoom += 0.05;
