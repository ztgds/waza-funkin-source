...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		var frameratePoop:Int = 24;
		if (curCharacter == 'bf-fastahh')
			frameratePoop = 4;
		...
		{		
			case 'bf' | 'bf-fastahh':
				...
				animation.addByPrefix('idle', 'BF idle dance', frameratePoop, false);
			...
			case 'waza':
				...
				frames = Paths.getSparrowAtlas('characters/waza', 'shared');
	...
	{
		if (!debugMode && canDance && !specialAnim)
		{
			switch (curCharacter)
			{
				case 'gf':
