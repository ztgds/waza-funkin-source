...
class PlayState extends MusicBeatState
{
	...
	public var allowCamHUDZoom:Bool = true;
	...
	var allowShaking:Bool = true;
	var minionNalgas:Bool = false;
	private var holaDani:Bool = false;
	private var elPepeEteSech:Bool = false;
	
	#if botplay
	var botPlayCanBeEnabled:Bool = true;
	var botplayTxt:FlxText;
	#end

	var chupaloo:Float = 2;
	var camBopVAL:Float = 0.015;
	var camHUDBopVAL:Float = 0.05;

	var beatToSnap:Float = 1;
	var snapToBeat:Float = 0;
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			case 'waza-pelicula':
				...
				sexTrail.color = 0xFFFF00FF;
				sexTrail.visible = false;
				insert(members.indexOf(bfGroup) - 1, sexTrail);
		...
		if (FlxG.save.data.songPosition /*&& !isGreetingsCutscene && !['five-nights', 'overdrive'].contains(SONG.song.toLowerCase())*/)
		{
			...
			songName = new FlxText(songPosBG.x, songPosBG.y, 0, "0:00 / 0:00", 32);
			...
			for (songPosShit in [songPosBG, songPosBar, songName]) {
				songPosShit.alpha = 0;
			}
		...
		#if botplay
		botplayTxt = new FlxText(0, 0);
		botplayTxt.setFormat(font, 26 * fontScaler, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		botplayTxt.borderSize = 1.5 * fontScaler;
	...
	public function createBackgroundSprites(bgName:String, revertedBG:Bool):FlxTypedGroup<FlxSprite>
	{
		...
		switch (bgName)
		{
			case 'waza':
		     	stageName = 'waza';
				bgZoom = 0.75;
				var bg:BGSprite = new BGSprite('bg', -600, -200, Paths.image('stages/waza/fondofeo'), null, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('stageFront', -650, 600,
		     		Paths.image('stages/waza/piso'), null, 0.9, 0.9);
			...
			case 'insano':
				...
				var bg:BGSprite = new BGSprite('bg', -600, -200, Paths.image('stages/insano/freefire'), null, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('stageFront', -650, 600,
				Paths.image('stages/insano/ffpiso'), null, 0.9, 0.9);
				stageFront.updateHitbox();
				add(stageFront);
			default:
				bgZoom = 0.9;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (isStoryMode) {
				switch (curStage.toLowerCase()) {
					case 'insano' | 'waza':
						FlxTween.tween(FlxG.camera, {zoom: bgZoom}, 1, {ease: FlxEase.backOut});
				}
			}
			...
			switch (swagCounter)
			{
				case 0:
					...
					// cleaner song pos bars intro??? //
					for (songPosShit in [songPosBG, songPosBar]) {
						songPosShit.scale.x = 1.155;
						songPosShit.scale.y = 1.155;
						FlxTween.tween(songPosShit, {alpha: 1}, 0.65, {ease: FlxEase.circOut});
						FlxTween.tween(songPosShit.scale, {x: 1, y: 1}, 0.85, {ease: FlxEase.circOut});
					}
				case 1:
	...
	function startSong():Void
	{
		...
		// cleaner song pos text transition??? //
	    if (songName != null)
		{
			songName.scale.x = 1.095;
			songName.scale.y = 1.095;
			FlxTween.tween(songName, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
			FlxTween.tween(songName.scale, {x: 1, y: 1}, 0.75, {ease: FlxEase.circOut});
		}
	...
	private function generateStaticArrows(player:Int, regenerate:Bool = false):Void
	{
		...
		//	{
				babyArrow.y -= 15;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {y: babyArrow.y + 15, alpha: 1}, 1, {
	...
	override public function update(elapsed:Float)
	{
		...
		// Updates Song Pos Text (Actualiza el texto de la duracion de la cancion) don francisco pagame la pension
		if (songName != null)
		{
			// Lo q dura la cancion :fire:
		    ...
		    FlxStringUtil.formatTime(songCalc / 1000, false)
			+ ' / ' 
			+ FlxStringUtil.formatTime(FlxG.sound.music.length / 1000, false); //thank u villezen
		}
		...
		//var currentBeat:Float = (Conductor.songPosition/1000)*(SONG.bpm / 140);

		if (minionNalgas)
		{
			for (what in [camHUD]) {
				camRotateAngle = -5 * Math.cos((elapsedtime * 2) + what.ID * 2);
			}
		...
		switch (SONG.song.toLowerCase())
		{
			case 'insanos':
				...
				{
				 	case 1:
						cinematicBars(5.25, 400);
					    showOnlyStrums();
						allowCamZoom = false;
					    FlxTween.tween(FlxG.camera, {zoom: 1.25}, 4.5, {ease: FlxEase.backOut});
				    case 64:
						FlxG.camera.flash();
					    restoreHUDElements();
					    allowCamZoom = true;
				    case 176:
					    defaultCamZoom = bgZoom + 0.15;
					    FlxTween.tween(camHUD, {alpha: 0.85}, 1.25, {ease: FlxEase.cubeInOut});
					    FlxTween.tween(blackScreendeez, {alpha: 0.95}, 1.25);
				    case 192:
					    defaultCamZoom = bgZoom;
				    ...
				    case 576:
					    camZoomSnap = false;
					    defaultCamZoom = bgZoom + 0.1;
				    case 688:
					    beatToSnap = 1;
					    camZoomSnap = true;
					    defaultCamZoom = bgZoom + 0.2;
				    case 704:
					    defaultCamZoom = bgZoom - 0.05;
					    camHUDBopVAL = 0.065;
					    camBopVAL = 0.025;
				    case 1087:
					    camHUD.zoom += 0.075;
				    ...
				    case 1090:
					    ...
					    allowCamZoom = false;
				    case 1091:
					    FlxTween.tween(FlxG.camera, {zoom: 1.05}, 25);
				    case 1327:
					    FlxTween.tween(blackScreendeez, {alpha: 1}, 1.25);
				    case 1343:
					    hideHealthBars();
				}
			case 'waza-pelicula':
				...
				switch(curStep)
				{
					...
					case 384:
						allowShaking = false;
						camZoomSnap = false;
						defaultCamZoom = 1.035;
						allowCamZoom = false;
						FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 2, 
						{
							ease: FlxEase.backOut,
					...
					case 1343:
						...
						holaDani = false;
					case 1439:
						for (wazaDad in [dad, evilTrail, iconP2]) {
							FlxTween.tween(wazaDad, {alpha: 0}, 5);
						}
					case 1567:
                        blackScreendeez.alpha = 1;
						FlxG.camera.alpha = 0;
				}
			case 'old-insanos': // case 'insanos':
	    		switch(curStep)
		    	{
					case 1088:
						camZoomSnap = true;
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					switch (SONG.song.toLowerCase()){
						case 'waza-pelicula':
							if(health > 0.1) health -= 0.0165;

							if (allowShaking) 
							{
								camHUD.shake(0.0025, 0.095);
								FlxG.camera.shake(0.0045, 0.065);
							}
			...
			if (focusondad)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'senpai-angry':
						...
						camFollow.x = dad.getMidpoint().x - 100;
					case 'insano':
						camFollow.x = dad.getMidpoint().x + 320;
						camFollow.y = dad.getMidpoint().y - 240;
					case 'waza':
						camFollow.y = dad.getMidpoint().y - 165;
				}
	...
	function endSong():Void
	{
		...
		for (songPosShit in [songPosBG, songPosBar, songName]) {
			FlxTween.tween(songPosShit, {alpha: 1}, 0.65, {ease: FlxEase.circOut});
			FlxTween.tween(songPosShit.scale, {x: 0, y: 0}, 0.45, {ease: FlxEase.circOut});
		}
	...
	private function keyShit():Void
	{
		...
		#if botplay
	    if(FlxG.keys.justPressed.SIX && botPlayCanBeEnabled) 
		{
	...
	function showOnlyStrums()
	{
		for (hudShit in [scoreTxt, kadeEngineWatermark, healthBarBG, healthBar, iconP1, iconP2]) {
			FlxTween.tween(hudShit, {alpha: 0}, 1, {ease: FlxEase.expoOut});
		}
	}
	function hideHealthBars()
	{
		for (healthStuffs in [healthBarBG, healthBar, iconP1, iconP2]) {
			FlxTween.tween(healthStuffs, {alpha: 0}, 1, {ease: FlxEase.expoOut});
		}
	}
	function restoreHUDElements()
	{
		for (hudShit in [scoreTxt, kadeEngineWatermark, healthBarBG, healthBar, iconP1, iconP2,
			songName, songPosBar, songPosBG]) 
		{
			if (hudShit.alpha == 0)
		    	FlxTween.tween(hudShit, {alpha: 1}, 1, {ease: FlxEase.expoOut});
			if (!hudShit.visible)
				hudShit.visible = true;
		}
	}
	...
	function cinematicBars(time:Float, closeness:Float)
	{
		...
		insert(members.indexOf(strumLineNotes), upBar);
		insert(members.indexOf(strumLineNotes), downBar);

		FlxTween.tween(upBar, {y: (FlxG.height - upBar.height) / 2 - closeness}, (1.5) / 2, {
			ease: FlxEase.expoInOut,
			onComplete: function(tween:FlxTween)
			{
				new FlxTimer().start(time, function(timer:FlxTimer)
				{
					FlxTween.tween(upBar, {y: upBar.y - 2000}, (1.5) / 2, {
						ease: FlxEase.expoIn,
						onComplete: function(tween:FlxTween)
						{
							remove(upBar);
		...
		FlxTween.tween(downBar, {y: (FlxG.height - downBar.height) / 2 + closeness}, (1.5) / 2, {
			ease: FlxEase.expoInOut,
			onComplete: function(tween:FlxTween)
			{
				new FlxTimer().start(time, function(timer:FlxTimer)
				{
					FlxTween.tween(downBar, {y: downBar.y + 2000}, (1.5) / 2, {
						ease: FlxEase.expoIn,
						onComplete: function(tween:FlxTween)
						{
							remove(downBar);
	...
	override function beatHit()
	{
		...
		if (healthBar.percent > 20 || healthBar.percent > 80)
			FlxTween.angle(iconP1, -10, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
		if (healthBar.percent < 80 || healthBar.percent < 20)
			FlxTween.angle(iconP2, 10, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
		// mr poop
		...
		// beatToSnap (def value is 1, snaps to every beat)
		// snapToBeat (def value is 0, snaps on 0 of the prev beat)
		if (camZoomSnap && camZooming && curBeat % beatToSnap == snapToBeat)
		{
			if(allowCamZoom) FlxG.camera.zoom += camBopVAL;
			if(allowCamHUDZoom)	camHUD.zoom += camHUDBopVAL;
		}
		...
		if (elPepeEteSech)
		{
		    if (curBeat % 4 == 0 || curBeat % 4 == 2) 
			{
				...
				if(allowCamZoom) FlxG.camera.zoom += 0.015;
			} 
			else if (curBeat % 4 == 1 || curBeat % 4 == 3) 
			{
				for (cameras in [camHUD, FlxG.camera]) {
