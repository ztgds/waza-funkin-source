...
class ClassHUD extends FlxTypedGroup<FlxBasic>
{
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(iconP1.width, 150, zoomLerp / (Init.trueSettings.get('Framerate Cap') / 60))));
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 9), 0, 1));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(iconP2.width, 150, zoomLerp / (Init.trueSettings.get('Framerate Cap') / 60))));
		iconP2.updateHitbox();
